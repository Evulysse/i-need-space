...
function passes(norad, latitude, longitude) {
    ...
    xmlHttpRequest.onload = function() {
        if(xmlHttpRequest.status === 200) {
            ...
            time[0].textContent = new Date(satellite.rise.utc_datetime);
            time[1].textContent = new Date(satellite.culmination.utc_datetime);
            time[2].textContent = new Date(satellite.set.utc_datetime);
            time[3].textContent = new Date(satellite.visible);
        }
    }
...
function search() {
    ...
    if(ACCESS_TOKEN && address && norad) {
        ...
        console.log("Please make sure you entered correct values for all the inputs.");
    }
}
